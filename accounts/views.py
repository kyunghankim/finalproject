...
def signup(request):
    ...
    if request.user.is_authenticated:
        return redirect('prediction_views:intro')

    if request.method == 'POST':
        # User 생성!
        form = UserCreationForm(request.POST)
        profile_form = ProfileForm(request.POST, request.FILES)
        if form.is_valid() and profile_form.is_valid(): #<- 유효성 2개 동시에 검사
            ...
            profile.save()
            # + Profile 생성
            # Profile.objects.create(user=user) <-위 3줄에서 역할 대체해서 주석처리
            # 3-1 저장했다면 User로 로그인!
            auth_login(request,user)
            # 4. 저장 결과 확인이 가능한 페이지로 
            return redirect('prediction_views:intro')
    else:
        # User 생성 양식 보여주기
...
def password(request):
    ...
    if request.method == 'POST':
        ...
        # Password 변경 양식 보여주기
        form = PasswordChangeForm(user) #<- 비밀번호를 고치려는 유저를 필요로함
    context = {
        'form': form,
    }
...
def profile_edit(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("accounts:profile_edit") #<- :인 이유는 앱이름:name(profile_edit)
    else:
        #profile 수정 양식 보여주기
...
def profile(request, username):
    ...
    profile = Profile.objects.get(user__username=username) #<- 여러 profile중에 username인 것을 get!
    ...
    return render(request, 'accounts/profile_detail.html', context) #<- profile_detail을 사용하는 함수들은 'profile'을 꼭 써야함
